...
Stash.prototype.get = function(key, dbFetch, cb) {
  ...
  self.memGet(key, dbFetch, function(err, result) {
    ...
    if (self.conf.cacheErrors && result instanceof Error) {
      ...
      var args = argToArray.call(arguments);
      args.shift();
...
Stash.prototype.fetch = function(key, dbFetch, cb) {
  ...
  if (!self.cacheRedis.connected && !self.conf.wait.redis) {
    return cb(self.cacheError(
      new Error('redis unavailable')
    ));
  }

  var cacheFetch = function(key, dbFetch, cb) {
    var got = function(err, content) {
      ...
      var lock = function(done) {
        ...
        self.warlock.lock(
          ...
          timed(self.conf.timeout.fetchLock, done)
        );
      };
      ...
      var lockCb = function(err, unlock) {
        ...
        if (unlock) {
          ...
          var dbFetchCb = function(err, result) {
            var content;
          ...
          self.log('db:fetch');
          return dbFetch(timed(self.conf.timeout.dbFetch, dbFetchCb));
        } else {
    ...
    self.cacheRedis.get(key, timed(self.conf.timeout.fetch, got));
  };

  self.metrics.increment('fetch');
  cacheFetch(key, dbFetch, function(err, result) {
...
Stash.prototype.del = function(key, cb) {
  ...
  self.cacheDel(key, timed(self.conf.timeout.del, function(err) {
