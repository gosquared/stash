...
Stash.prototype.cacheFetch = function(key, dbFetch, retries, cb) {
  ...
  var done = function swap(err){
    ...
    // any result will bypass the error cache and be stored in result
    // cache regardless of if there is an err.
    // If this is not desired then fetchFn should not pass any
    // result args to its callback when err exists.
