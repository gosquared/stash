...
var Stash = function(createRedisClient, opts) {
  ...
  var conf = {
    ...
    memoize: {              // in-memory cache options
      result: {
        ...
        max: 100000         // max number of items in results cache
      },
      error: {
        ttl: 5000,          // ttl of errors cache
        max: 100000         // max number of items in error cache
      }
    },
  ...
  self.memoize.result = memoize(self.getError.bind(self), {
    async: true,
    primitive: true,
    maxAge: self.conf.memoize.result.ttl,
    max: self.conf.memoize.result.max,
    resolvers: [String],
    length: 1
  });
  ...
  self.memoize.err = memoize(self.cacheFetch.bind(self), {
    async: true,
    primitive: true,
    maxAge: self.conf.memoize.error.ttl,
    max: self.conf.memoize.error.max,
    resolvers: [String],
    length: 1
  });
...
Stash.prototype.cacheFetch = function(key, dbFetch, retries, cb) {
  ...
  var done = function swap(err, result){
    // result is array of dbFetch result args
    // masquerade result as an error so it bypasses error cache
    // err or result must be null otherwise we'd bypass both caches
    if (err) result = null;
    else err = null;
    cb(result, err);
  };
    ...
    lock(function(err, unlock) {
      ...
      return dbFetch(function(err) {
        ...
        try {
          ...
          err.exception = e;
          return unlock(function() {
            // Will not memoize if encode error
            done(err, result);
          });
        }
        ...
        self.cacheRedis.multi()
          ...
          .exec(function(err) {
            ...
            unlock(function() {
              // Will memoize result even if failed to save to cache
              done(err, result);
            });
          });
      });
